...
import ChatMessage from "@/components/chatbot/ChatMessage";
import { sendMessageToChatbot } from "@/api";
import { useParams } from "react-router-dom"; // ✅ Change 1: Import useParams
...
export default function ChatbotView() {
    const { roomId } = useParams(); // ✅ Change 2: Get roomId from URL
    const [messages, setMessages] = useState<Message[]>([]);
    const [input, setInput] = useState("");
    const [loading, setLoading] = useState(false);
    const chatEndRef = useRef<HTMLDivElement>(null);
    ...
    const handleSend = async () => {
        ...
        try {
            // ✅ Change 3: Pass roomId and add type to chunk
            await sendMessageToChatbot(
                userMessage.content, // Message
                roomId || "",        // Room ID (Passed to API)
                (chunk: string) => { // Callback with type definition
                    setMessages((prev) => {
                        const lastMessage = prev[prev.length - 1];
                        if (lastMessage?.sender === "ai") {
                            return [
